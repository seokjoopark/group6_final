...
class SAC(OffPolicyRLModel):
    ...
    def learnPerStep(self, total_timesteps, callback=None,
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for step in range(total_timesteps):
                ...
                [new_obs, reward, done, info] = yield
    ...
    def learn(self, total_timesteps, callback=None,
              log_interval=100, tb_log_name="SAC", reset_num_timesteps=True, replay_wrapper=None, qs=None, qr=None, main_thread=False, gui=False, mutex=None, thread_status=False, cond=None, change_value=None):
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for step in range(total_timesteps):
                ...
                if qs is not None:
                    # if main_thread == True:
                    #     thread_step = threading.Thread(target=self.env.step, args=([0]))
                    #     thread_step.start()
                    # else:
                    #     thread_step = threading.Thread(target=self.env.step, args=([1]))
                    #     thread_step.start()
                    # print("-=-", unscaled_action)
                    # qs.put(unscaled_action)
                    # new_obs, reward, done, info = qr.get()
                    if main_thread == True:
                        unscaled_action = np.append(unscaled_action, [0])
                    else:
                        unscaled_action = np.append(unscaled_action, [1])
                    
                    new_obs, reward, done, info = self.env.step(unscaled_action)
                    # if done:
                        # print(done, "DONE MAIN THREAD?", main_thread)
                    if main_thread == True:
                        # print(info['sleep1'], "MAIN THREAD?", main_thread)
                        if info['sleep1'] == True:
                            sleep = True
                    else:
                        # print(info['sleep2'], "MAIN THREAD?", main_thread)
                        if info['sleep2'] == True:
                            sleep = True
