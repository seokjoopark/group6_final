...
class Monitor(Wrapper):
    ...
    def __init__(self, env, filename, allow_early_resets=True, reset_keywords=(), info_keywords=()):
        ...
        if filename is None:
            ...
            log_dir = logDir()
            if not os.path.isdir(log_dir + env.args.prefix):
                os.makedirs(log_dir + env.args.prefix, exist_ok=True)
            self.file_handler = open(filename, "wt")
            
            self.file_handler.write('#%s\n' % json.dumps({"t_start": self.t_start, 'env_id': env.spec and env.spec.id, 'args':vars(env.args)}))
